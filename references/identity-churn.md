...
Use locals to hint explicit resource deletion order and avoid hidden dependency issues:

```hcl
# Forces Terraform to delete subnets before secondary CIDR blocks
locals {
  vpc_id = try(
...
# Without local: Terraform might delete CIDR before subnets → ERROR
# With local: Subnets deleted first, then CIDR, then VPC ✓
```
