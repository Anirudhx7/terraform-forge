...
- [ ] Consider cross-variable validation (1.9+)
- [ ] Replace `null_resource` with `terraform_data` (1.4+)
...
├─ Resource addressing (count → for_each)
│  └─ Use: moved blocks + for_each conversion
├─ Secrets in state
│  └─ Use: external secret manager + write-only arguments (1.11+)
