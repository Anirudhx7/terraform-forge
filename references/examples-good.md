...
Why this works:
- consistent tagging without repeating in every resource
- overridable per-resource when needed

## 10) State backend with locking
...
- scoped key per stack/environment

## 11) Least-privilege IAM with multiple statements

```hcl
resource "aws_iam_policy" "app" {
  name        = "${var.project}-app-policy"
  description = "Minimal permissions for application workload"
