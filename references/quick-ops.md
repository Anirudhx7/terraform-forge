...
| Enterprise | HCP Terraform, TF Cloud | Multiple vendors |
| Migration | N/A | Drop-in for Terraform ≤1.5 |

Choose Terraform if: using HCP/TF Cloud, need enterprise support, want latest features first.
Choose OpenTofu if: prefer open governance, BSL doesn't fit, community-driven development.

Commands are interchangeable — replace `terraform` with `tofu`.
